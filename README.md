...
- **🎓 AI Expert 뷰** — AI 전문가 인증 대상 **13개 최상위 학회**(ICML, ICLR, AAAI, NeurIPS, IJCAI, CVPR, ICCV, ECCV, ACL, EMNLP, NAACL, ICASSP, INTERSPEECH)를 조회. **기준 연도 메뉴**로 연도별 대상 리스트를 선택하고, 선택 연도의 **컨퍼런스 리스트 등재/미등재**를 요약(예: 2026년 하반기 기준 ICASSP·INTERSPEECH 미등재)하며, 13개 학회의 **월별 제출 일정**을 함께 열람 (데이터: [`docs/data/ai_expert.json`](docs/data/ai_expert.json))
...
│   │   ├── ai_expert.json       # 연도별 AI Expert 대상 학회(13개) 목록
